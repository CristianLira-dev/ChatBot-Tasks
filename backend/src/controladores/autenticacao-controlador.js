  ...
  return {
    cadastrar: async (req, res) => {
      ...
      const usuario = await repositorio.criarUsuario({
        ...dados,
        senhaCriptografada: await gerarHashSenha(dados.senha)
      });

      return res.status(201).json({
        usuario: removerSegredos(usuario),
    ...
    entrar: async (req, res) => {
      ...
      if (
        !usuario ||
        !(await compararSenha(dados.senha, usuario.senhaCriptografada))
      ) {
